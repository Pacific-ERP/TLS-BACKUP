...
class AccountFollowupReport(models.AbstractModel):
    ...
    def send_email(self, options):
        ...
        _logger.error(f"[send_email] : company {company.name if company else False} doc {company.technical_data if company else False}")
        ...
        if email and email.strip():
            ...
            partner.with_context(mail_post_autofollow=True, lang=partner.lang or self.env.user.lang).message_post(
                ...
                attachment_ids= company.technical_data.ids or [],
            )
            return True
